...
# 記事を作成（カスタムオプション）
npm run zenn-new -- --slug my-article --title "記事タイトル" --type tech --emoji 📝

# 本を作成
npm run zenn-new -- book --slug my-book

# プレビュー（カスタムポート、ブラウザ自動起動）
npm run zenn-preview -- --port 3000 --open

# 記事一覧を表示
npm run zenn-list

# 本一覧を表示
npm run zenn-list -- books
```

##### 方法 2: グローバルコマンドを使う - Zenn
...
##### 方法 1: npm scripts を使う（推奨）- Qiita

```bash
# 新しい記事を作成
npm run qiita-new my-first-article

# プレビューを起動（カスタムポート）
npm run qiita-preview -- --port 3000

# 記事を公開
npm run qiita-publish my-first-article

# すべての記事を公開
npm run qiita-publish-all

# Qiita 上の更新を同期
npm run qiita-pull -- --root ./my-articles

# 詳細ログを表示しながら公開
npm run qiita-publish -- my-article --verbose

# 認証
npm run qiita-login -- --credential ~/.qiita-credentials
```

##### 方法 2: グローバルコマンドを使う - Qiita
...
# 特定の blog ID から記事を取得
npm run hatena-pull -- fluxrozin.hateblo.jp
